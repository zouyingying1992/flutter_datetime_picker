...
class DateTimePickerModel extends CommonPickerModel {
  ...
  DateTimePickerModel(
    ...
    if (currentTime != null) {
      ...
      if (this.minTime != null && this.maxTime.isAfter(now)) {
    ...
    if (this.minTime != null &&
        this.maxTime != null &&
        this.maxTime.isBefore(this.maxTime)) {
      // invalid
      this.minTime = null;
      this.maxTime = null;
    }
    ...
    if (this.minTime != null && isAtSameDay(this.maxTime, this.currentTime)) {
      _currentMiddleIndex = this.currentTime.hour - this.maxTime.hour;
      if (_currentMiddleIndex == 0) {
        _currentRightIndex = this.currentTime.minute - this.maxTime.minute;
      }
    }
  }
  ...
  void setLeftIndex(int index) {
    ...
    if (isAtSameDay(minTime, time)) {
      var index = min(24 - maxTime.hour - 1, _currentMiddleIndex);
  ...
  void setMiddleIndex(int index) {
    ...
    if (isAtSameDay(minTime, time) && index == 0) {
      var maxIndex = 60 - maxTime.minute - 1;
  ...
  String middleStringAtIndex(int index) {
    if (index >= 0 && index < 24) {
      DateTime time = currentTime.add(Duration(days: _currentLeftIndex));
      if (isAtSameDay(minTime, time)) {
        if (index >= 0 && index < 24 - maxTime.hour) {
          return digits(maxTime.hour + index, 2);
        } else {
          return null;
        }
      } else if (isAtSameDay(maxTime, time)) {
  ...
  String rightStringAtIndex(int index) {
    if (index >= 0 && index < 60) {
      ...
      if (isAtSameDay(minTime, time) && _currentMiddleIndex == 0) {
        if (index >= 0 && index < 60 - maxTime.minute) {
          return digits(maxTime.minute + index, 2);
        } else {
          return null;
        }
      } else if (isAtSameDay(maxTime, time) &&
